...
form.addEventListener('submit',(event)=>{
 ...
 console.log(value);
 axios.get(`https://api.openweathermap.org/data/2.5/forecast?q=${value}&appid=6cde75716bf7f47ec3640739431eaba6`)
    .then((res)=>{
        
        let min=res.data.list[0].main.temp_min
        let max=res.data.list[0].main.temp_max
        let temp=(min+max)/2-273.15;
...
async function getlocation() {
    ...
    console.log(navigator.geolocation.getCurrentPosition(showLoc));
}
